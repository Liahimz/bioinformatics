...
{
    for (int i = 1; i <= n; ++i) {
        grid[i][0] = std::max(grid[i - 1][0] + sigma, 0);
    ...
    for (int i = 1; i <= m; ++i) {
        grid[0][i] = std::max(grid[0][i - 1] + sigma, 0);
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1;  j <= m; ++j) {
            ...
            grid[i][j] = std::max(std::max(grid[i - 1][j - 1] + tmp, 0), std::max(grid[i - 1][j] + sigma, grid[i][j - 1] + sigma));
...
void output_backtrack(std::vector<std::vector<int>> &back_track, std::string pattern_1, std::string pattern_2, int start_i, int start_j) {
    int i = start_i;
    int j = start_j;
    std::string ans_1 = "";
    std::string ans_2 = "";
...
int main() {
    std::string filename("PAM250.txt");
    ...
    output_backtrack(back_track, pattern_1, pattern_2, max_i, max_j);
